...
``XDG_RUNTIME_DIR`` is the one XDG base directory with *no* specified default. The spec says:
...
uses a per-user directory under the system temp dir, keyed to the uid so a shared ``/tmp``
...
from credbox.environment import read_absolute_path_override
from credbox.paths import app_dir_segment
from credbox.permissions import ensure_private_dir
...
def runtime_dir(app: str, *, create: bool = True) -> Path:
    ...
    directory is created if absent and verified to be a real directory owned by this user with
    mode 0700 -- so a socket or lock file written inside it is reachable only by its owner.
    With ``create=False`` the path is computed and returned without touching the filesystem
    (for display), and no security guarantee is made.

    Raises:
        InvalidAppNameError: ``app`` is not a valid directory segment.
        InsecureStorageError: ``create`` and a directory on the path exists but is a symlink or
            owned by another user (a hijack attempt in a shared temp directory).
        CredBoxError: ``create`` and the directory could not be created.
    """
    segment = app_dir_segment(app)
    base = read_absolute_path_override("XDG_RUNTIME_DIR")
    if base is not None:
        path = base / segment
...
def _fallback_runtime_root() -> Path:
    ...
    ``<tempdir>/credbox-<uid>`` on POSIX (the uid keeps a shared ``/tmp`` from being
    hijacked), ``<tempdir>/credbox`` elsewhere (Windows temp is already per-user)."""
    system_temp_dir = Path(tempfile.gettempdir())
    if os.name == "posix":
        return system_temp_dir / f"credbox-{os.getuid()}"
    return system_temp_dir / "credbox"
