...
비밀을 저장할 때 빈 값(빈 문자열 또는 공백만)은 거부하고 앞뒤 공백은 잘라내므로, 저장된 키가
해석 결과와 일치하고 붙여넣은 키의 끝 개행이 디스크에 남지 않습니다.
...
- **키링을 쓸 수 없을 때**: 키링이 *아예 없으면*(설치 안 됨 · 서버라 백엔드 없음) 모든 작업이
  자동으로 파일 저장소를 대신 씁니다(폴백), 경고가 한 번 떠서 "지금 값이 파일에 저장됐다"는
  걸 알려줍니다. 키링이 *있는데 실패하면*(예: 잠김) `get` / `set`은 여전히 파일로 폴백하지만,
  `unset`은 조용히 폴백하지 않고 큰 소리로 실패합니다(예외) -- 키링에 아직 남아 있을지 모르는
  시크릿을 "지웠다"고 잘못 보고하지 않기 위해서입니다.
...
마스킹. 작성자의 패키지는 자기 도메인 설정(계정, 라우트, 토픽)은 그대로 두고, 그
